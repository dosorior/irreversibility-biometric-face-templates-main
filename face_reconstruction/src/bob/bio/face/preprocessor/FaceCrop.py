...
class FaceCrop():
    ...
    def transform(self, X, annotations=None):
        ...
        def _crop(image, annot):
            ...
            # crop face
            return self.cropper.transform(image, annot)
...
class BoundingBoxAnnotatorCrop():
    """
    ...
    def transform(self, X, annotations=None):
        ...
        for x, annot in zip(X, annotations):
            ...
            # If nothing was detected OR if the annotations are swapped, return the cropped face
            if (
                ...
                logger.warning(
                    f"Unable to detect face in bounding box. Got : {annotator_annotations}. "
                    f"Cropping will be only based on bounding-box."
                )
