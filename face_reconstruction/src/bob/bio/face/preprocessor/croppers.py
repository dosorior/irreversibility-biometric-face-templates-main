...
class FaceEyesNorm(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X, annotations=None):
        ...
        cropped_image = X[..., ::-1, :, :] if X.ndim > 2 else X
        cropped_image = np.moveaxis(cropped_image, -3, -1)
        ...
        expanded_image = (
            expanded_image[..., ::-1] if X.ndim > 2 else expanded_image
        )

        expanded_image = np.moveaxis(expanded_image, -1, -3)
